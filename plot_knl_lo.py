...
#		fig = plt.figure(figsize=(8, 4))
		...
		for i in xrange(N_KERNL):
			p1[i] = ax1.bar(ind+i*width, data_set_1[j][i], width, color=colors[i], hatch=hatches[i])
			
		for i in xrange(N_THREADS-5):	
			ax1.text(i, data_set_1[j][0][i]+.1, ' %.1f'%data_set_1[j][0][i], va='bottom', fontsize=11, ha='left', rotation=90)	
			ax1.text(i+1*width, data_set_1[j][1][i]+.1, ' %.1f'%data_set_1[j][1][i], va='bottom', fontsize=11, ha='left', rotation=90)	
			ax1.text(i+2*width, data_set_1[j][2][i]+.1, ' %.1f'%data_set_1[j][2][i], va='bottom', fontsize=11, rotation=90)	
#			ax1.text(i+3*width, data_set_1[j][3][i]+.1, ' %.1f'%data_set_1[j][3][i], va='bottom', fontsize=9.5, rotation=90)	
#			ax1.text(i+4*width, data_set_1[j][4][i]+.1, ' %.1f'%data_set_1[j][4][i], va='bottom', fontsize=9.5, rotation=90)	
#			ax1.text(i+5*width, data_set_1[j][5][i]+.1, ' %.1f'%data_set_1[j][5][i], va='bottom', fontsize=9.5, rotation=90)	

		ax1.set_xticks(ind+i*width)
		ax1.set_xticklabels(['1', '2', '4', '6', '8', '16', '32', '64', '128', '256'], ha='right')
#		ax1.set_xticklabels(['16', '32', '64', '128', '256'], ha='right')
